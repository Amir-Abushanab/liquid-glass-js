/**
 * The hero's install command: a package-manager selector over one `add` line.
 *
 * Shares `preferred-pm` in localStorage and the `pm-change` event with the registry's
 * React command blocks, so picking pnpm here picks it on every component page too —
 * one choice, remembered, wherever you land first.
 *
 * Brand marks are the official simple-icons paths (CC0-1.0, no attribution required),
...
document.querySelectorAll('[data-install]').forEach((root) => {
  ...
  line.innerHTML =
    '<code class="pm__cmd"><span class="pm__prompt" aria-hidden="true">$</span> <span class="pm__text"></span></code>' +
    '<button class="pm__copy" type="button" aria-label="Copy install command">' +
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' +
    '<rect x="9" y="9" width="13" height="13" rx="2"></rect>' +
    '<path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>' +
    '<svg class="pm__tick" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' +
    '<path d="M20 6 9 17l-5-5"></path></svg></button>';

  root.append(tabs, line);
  ...
  copy.addEventListener('click', async () => {
    try {
      await navigator.clipboard.writeText(`${MANAGERS[current].cmd} ${PKG}`);
      copy.classList.add('is-copied');
      clearTimeout(reset);
      reset = window.setTimeout(() => copy.classList.remove('is-copied'), 1500);
    } catch {
      /* clipboard blocked: leave the command visible to select by hand */
    }
  });

  window.addEventListener('pm-change', paint);

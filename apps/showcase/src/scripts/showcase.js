...
const CFG_ICONS = {
  segmented: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="1.5" y="4.5" width="13" height="7" rx="3.3"/><line x1="8" y1="4.7" x2="8" y2="11.3"/></svg>',
  slider: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><line x1="2" y1="8" x2="14" y2="8"/><circle cx="6" cy="8" r="2.4" fill="currentColor" stroke="none"/></svg>',
  switch: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="1.5" y="5" width="13" height="6" rx="3"/><circle cx="11" cy="8" r="2" fill="currentColor" stroke="none"/></svg>',
  ...
  ripple: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><circle cx="8" cy="8" r="1.5" fill="currentColor" stroke="none"/><circle cx="8" cy="8" r="4" opacity="0.85"/><circle cx="8" cy="8" r="6.3" opacity="0.4"/></svg>',
  ...
  button: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="1.5" y="5" width="13" height="6" rx="3"/><line x1="5.2" y1="8" x2="10.8" y2="8"/></svg>',
  dropdown: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="2.5" y="2" width="11" height="3.6" rx="1.8"/><rect x="2.5" y="8" width="11" height="6" rx="2" opacity="0.55"/></svg>',
  shape: '<svg viewBox="0 0 16 16" fill="currentColor" stroke="none"><path d="M8 1c.7 4 3 6.3 7 7-4 .7-6.3 3-7 7-.7-4-3-6.3-7-7 4-.7 6.3-3 7-7z"/></svg>',
  emoji: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><circle cx="8" cy="8" r="1.8" fill="currentColor" stroke="none"/><line x1="8" y1="1.5" x2="8" y2="3.6"/><line x1="8" y1="12.4" x2="8" y2="14.5"/><line x1="1.5" y1="8" x2="3.6" y2="8"/><line x1="12.4" y1="8" x2="14.5" y2="8"/><line x1="3.5" y1="3.5" x2="5" y2="5"/><line x1="11" y1="11" x2="12.5" y2="12.5"/><line x1="12.5" y1="3.5" x2="11" y2="5"/><line x1="5" y1="11" x2="3.5" y2="12.5"/></svg>',
  paths: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><rect x="1.5" y="3" width="8" height="5.5" rx="1.4"/><rect x="4" y="5.2" width="8" height="5.5" rx="1.4" opacity="0.6"/><rect x="6.5" y="7.4" width="8" height="5.5" rx="1.4" opacity="0.35"/></svg>'
};
const FONT_PARAMS = [
...
const SEG_OPTS = { radius: 999, depth: 10, dome: 12, edge: 0.8, glow: 0.28, strength: 1, chroma: 0.22, blur: 0, shade: 0 };
const segLenses = [];
document.querySelectorAll("[data-seg]").forEach((seg) => {
  const labels = seg.querySelector(".seg__labels");
  const opts = Array.from(seg.querySelectorAll(".seg__opt"));
  if (!labels || !opts.length)
    return;
  const n = opts.length;
  const geom = () => {
  ...
  let g = geom();
  const lens = mountGlassLens({ target: labels, host: seg, lensW: g.pillW, lensH: g.pillH, ...SEG_OPTS });
  segLenses.push(lens);
  let lensX = 0, tweenRaf = 0;
  const segEase = cubicBezier(0.34, 1.35, 0.5, 1);
  const moveLensTo = (targetX) => {
    cancelAnimationFrame(tweenRaf);
    const from = lensX, t0 = performance.now(), dur = 300;
    const step = (now) => {
...
if (segLenses.length) {
  cfgSections.push({ id: "segmented", label: "Segmented", icon: CFG_ICONS.segmented, params: LENS_PARAMS, opts: { ...SEG_OPTS }, apply: (patch) => segLenses.forEach((l) => l.reconfigure(patch)) });
}
...
if (animGated.length && "IntersectionObserver" in window) {
  const animIO = new IntersectionObserver(
    (entries) => entries.forEach((e) => e.target.classList.toggle("is-anim-off", !e.isIntersecting)),
    { rootMargin: "100px" }
  );
...
if (pathGlSrc && pathGlCtx) {
  ...
  const pfx = ["svg", "frost"].map((k) => document.querySelector(`[data-pathfx="${k}"]`)).filter(Boolean).map((c) => ({ c, x: c.getContext("2d") }));
  const pathBtn = document.querySelector("[data-pathspawn]");
  const sizeC = (c) => { const r = c.getBoundingClientRect(); c.width = Math.max(1, Math.round(r.width * gdpr)); c.height = Math.max(1, Math.round(r.height * gdpr)); };
  const sizeAll = () => { sizeC(pathGlSrc); pfx.forEach(({ c }) => sizeC(c)); };
  sizeAll();
  new ResizeObserver(sizeAll).observe(pathGlSrc);
  let glVis = true, glRaf = 0;
  ...
  const nyanStars = Array.from({ length: 22 }, (_, i) => ({ x: (i * 61 % 100) / 100, y: (i * 37 % 100) / 100, tw: i % 4 }));
  const drawGl = () => {
    const w = pathGlSrc.width, h = pathGlSrc.height, ctx = pathGlCtx, t = performance.now() * 0.001;
    ctx.globalCompositeOperation = "source-over";
    ctx.fillStyle = "#1a1147"; ctx.fillRect(0, 0, w, h);                         // deep space
    ctx.fillStyle = "rgba(190,180,255,0.85)";                                    // twinkling scrolling stars
    const wrap = w + 40;
    for (const s of nyanStars) {
      const sx = ((s.x * wrap - t * 70) % wrap + wrap) % wrap - 20, sy = s.y * h;
      const r = h * (0.013 + 0.009 * (0.5 + 0.5 * Math.sin(t * 4 + s.tw)));
      ctx.fillRect(sx - r, sy - r * 0.32, r * 2, r * 0.64); ctx.fillRect(sx - r * 0.32, sy - r, r * 0.64, r * 2);
    }
    const bob = Math.sin(t * 5.5) * h * 0.045, midY = h * 0.52 + bob, catX = w * 0.64;
    // rainbow trail: 6 stripes + scrolling shade columns for the "flow"
    const cols = ["#ff2244", "#ff9922", "#ffee33", "#3ad633", "#33aaff", "#9a55ff"];
    const bandH = h * 0.34, sh = bandH / 6, top = midY - bandH / 2;
    for (let i = 0; i < 6; i++) { ctx.fillStyle = cols[i]; ctx.fillRect(0, top + i * sh, catX, sh + 1); }
    ctx.fillStyle = "rgba(0,0,0,0.14)";
    const cseg = Math.max(10, Math.round(w * 0.032));
    for (let x = -cseg * 2; x < catX; x += cseg * 2) ctx.fillRect(x - ((t * 130) % (cseg * 2)), top, cseg, bandH);
    // pop-tart body
    const bw = w * 0.19, bh = h * 0.36, bx = catX - bw * 0.3, by = midY - bh / 2;
    ctx.fillStyle = "#f7c99e"; ctx.fillRect(bx, by, bw, bh);
    ctx.fillStyle = "#ff9dc6"; ctx.fillRect(bx + bw * 0.11, by + bh * 0.12, bw * 0.78, bh * 0.76);
    const spr = ["#ff3b6b", "#33c6ff", "#ffe14d", "#4dff88"];
    for (let i = 0; i < 12; i++) { ctx.fillStyle = spr[i % 4]; ctx.fillRect(bx + bw * 0.16 + (i * 53 % 100) / 100 * bw * 0.64, by + bh * 0.2 + (i * 71 % 100) / 100 * bh * 0.56, bw * 0.05, bw * 0.05); }
    ctx.fillStyle = "#8f8f8f"; for (let i = 0; i < 4; i++) ctx.fillRect(bx + bw * (0.12 + i * 0.24), by + bh - 1, bw * 0.11, bh * 0.16);
    // head
    const hw = h * 0.27, hh = h * 0.25, hx = catX + bw * 0.5, hy = midY - hh / 2;
    ctx.fillStyle = "#9a9a9a";
    ctx.beginPath(); ctx.moveTo(hx + hw * 0.06, hy + 2); ctx.lineTo(hx + hw * 0.02, hy - hh * 0.24); ctx.lineTo(hx + hw * 0.34, hy + 2); ctx.fill();
    ctx.beginPath(); ctx.moveTo(hx + hw * 0.66, hy + 2); ctx.lineTo(hx + hw * 0.98, hy - hh * 0.24); ctx.lineTo(hx + hw * 0.94, hy + 2); ctx.fill();
    ctx.fillRect(hx, hy, hw, hh);
    ctx.fillStyle = "#333"; ctx.fillRect(hx + hw * 0.22, hy + hh * 0.34, hw * 0.12, hh * 0.2); ctx.fillRect(hx + hw * 0.64, hy + hh * 0.34, hw * 0.12, hh * 0.2);
    ctx.fillStyle = "#ff90b3"; ctx.beginPath(); ctx.arc(hx + hw * 0.2, hy + hh * 0.64, hw * 0.09, 0, 6.283); ctx.fill(); ctx.beginPath(); ctx.arc(hx + hw * 0.8, hy + hh * 0.64, hw * 0.09, 0, 6.283); ctx.fill();
    ctx.strokeStyle = "#333"; ctx.lineWidth = Math.max(1, h * 0.007); ctx.beginPath(); ctx.arc(hx + hw * 0.5, hy + hh * 0.56, hw * 0.15, 0.12 * Math.PI, 0.88 * Math.PI); ctx.stroke();
  };

  // emoji particle field rendered to an offscreen canvas (~a card's size)
  const pr = pathGlSrc.getBoundingClientRect();
  const LW = Math.max(120, Math.round(pr.width)), LH = Math.max(80, Math.round(pr.height));
  const off = document.createElement("canvas");
  off.width = Math.round(LW * gdpr); off.height = Math.round(LH * gdpr);
  const octx = off.getContext("2d");
  const SET = [
    ["🎉", 3, true], ["🎊", 2, true], ["🥳", 2, true], ["🤩", 1, true], ["😍", 1, true],
    ["✨", 3, false], ["⭐", 2, false], ["🌟", 2, false], ["💫", 1, true], ["💥", 1, false],
    ["⚡", 1, true], ["🔥", 2, false], ["🚀", 2, true], ["🌈", 2, false], ["🦄", 1, true],
    ["💜", 2, false], ["❤️", 1, false], ["🧡", 1, false], ["💛", 1, false], ["💚", 1, false],
    ["💙", 1, false], ["🩷", 1, false], ["🩵", 1, false], ["💖", 2, false], ["💕", 1, false],
    ["🙌", 1, true], ["👏", 1, true], ["👍", 1, true], ["🫶", 1, false], ["🦋", 1, true],
    ["🌸", 1, false], ["🌺", 1, true], ["🌼", 1, false], ["🍀", 1, false], ["💎", 1, false],
    ["🫧", 1, false], ["💧", 1, false], ["🍭", 1, true], ["🎈", 1, true], ["👾", 1, false],
    ["🇵🇸", 20, false], ["✊", 2, false], ["💪", 2, true], ["🐉", 2, true], ["🐲", 1, false],
  ...
  const cache = new Map();
  const raster = (e) => {
    let c = cache.get(e); if (c) return c;
    const px = Math.ceil(56 * gdpr), sz = Math.ceil(px * 1.4);
    c = document.createElement("canvas"); c.width = c.height = sz;
    const x = c.getContext("2d"); x.textAlign = "center"; x.textBaseline = "middle";
    x.font = `${px}px "Apple Color Emoji","Segoe UI Emoji",serif`; x.fillText(e, sz / 2, sz / 2);
    cache.set(e, c); return c;
  };
  const parts = [];
  const drawParticles = (dt) => {
    ...
    for (let i = parts.length - 1; i >= 0; i--) {
      ...
      p.x += p.xv * dt; p.y += p.yv * dt; p.yv += 0.03 * dt; p.xv *= Math.pow(0.99, dt); p.rot += p.rotv * dt;
      const t = p.life / p.maxLife, pop = Math.min(1, p.life / 6);
      const opacity = t < 0.12 ? t / 0.12 : t > 0.6 ? Math.max(0, (1 - t) / 0.4) : 1;
      const s = p.size * pop * gdpr, dir = p.flip ? -1 : 1;
      octx.setTransform(Math.cos(p.rot) * dir, Math.sin(p.rot) * dir, -Math.sin(p.rot), Math.cos(p.rot), p.x * gdpr, p.y * gdpr);
      octx.globalAlpha = opacity; octx.drawImage(raster(p.e), -s / 2, -s / 2, s, s);
    }
  ...
  const spawn = (count) => {
    for (let i = 0; i < count; i++) {
      ...
      parts.push({ x: LW / 2 + (Math.random() - 0.5) * LW * 0.72, y: LH + 8, xv: (Math.random() - 0.5) * 2.2, yv: -(2.6 + Math.random() * 2.4), rot: (Math.random() - 0.5) * 0.5, rotv: (Math.random() - 0.5) * 0.09, size: 22 + Math.random() * 22, life: 0, maxLife: 80 + Math.random() * 46, flip: pick.f && Math.random() < 0.5, e: pick.e });
    }
  };

  // one loop: Nyan backdrop (always) + emoji field blitted through all three paths (when active)
  let lastT = 0, wasActive = false;
  const loop = (now = performance.now()) => {
    glRaf = 0;
    if (!glVis) { lastT = 0; return; } // off-screen: stop; the observer restarts it on entry
    const dt = lastT ? Math.min((now - lastT) / 16.6667, 3) : 1; lastT = now;
    if (!reduce) drawGl();
    const active = parts.length > 0;
    if (active) {
      drawParticles(dt);
      pathGlCtx.drawImage(off, 0, 0, pathGlSrc.width, pathGlSrc.height);
      for (const { c, x } of pfx) { x.setTransform(1, 0, 0, 1, 0, 0); x.clearRect(0, 0, c.width, c.height); x.drawImage(off, 0, 0, c.width, c.height); }
      wasActive = true;
    } else if (wasActive) {
      for (const { c, x } of pfx) { x.setTransform(1, 0, 0, 1, 0, 0); x.clearRect(0, 0, c.width, c.height); }
      wasActive = false;
    }
    glRaf = requestAnimationFrame(loop);
  };
  drawGl();
  new IntersectionObserver((es) => { glVis = es[0].isIntersecting; if (glVis && !glRaf) glRaf = requestAnimationFrame(loop); }).observe(pathGlSrc);
  ...
  let holding = false, holdStart = 0, lastEmit = 0, holdRaf = 0;
  const holdLoop = (now) => {
    if (!holding) { holdRaf = 0; return; }
    const held = now - holdStart, cadence = Math.max(45, 110 - held / 18);
    if (now - lastEmit >= cadence && parts.length < 220) { lastEmit = now; spawn(reduce ? 1 : Math.min(5, 3 + Math.floor(held / 800))); }
    holdRaf = requestAnimationFrame(holdLoop);
  };
  const startHold = () => { if (holding) return; holding = true; holdStart = performance.now(); lastEmit = 0; spawn(reduce ? 4 : 8); if (!holdRaf) holdRaf = requestAnimationFrame(holdLoop); };
  const endHold = () => { holding = false; };
  if (pathBtn) {
    pathBtn.addEventListener("pointerdown", (e) => { e.preventDefault(); startHold(); });
    window.addEventListener("pointerup", endHold);
    pathBtn.addEventListener("pointercancel", endHold);
    pathBtn.addEventListener("keydown", (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); if (!e.repeat) startHold(); } });
    pathBtn.addEventListener("keyup", (e) => { if (e.key === "Enter" || e.key === " ") endHold(); });
  }
}
...
if (isld && isldTrack && isldThumb) {
  const TW = 48, TH = 44, TOP = 6, MARGIN = 4, S = 1.14;
  const LW = Math.round(TW * S), LH = Math.round(TH * S);
  let value = 0.5, dragging = false;
  ...
  const fromX = (clientX) => {
    const r = isld.getBoundingClientRect();
    value = Math.max(0, Math.min(1, (clientX - r.left - MARGIN - TW / 2) / (r.width - TW - 2 * MARGIN)));
    place();
  };
  const glass = () => {
...
if (isw && iswTrack && iswThumb) {
  const TRAVEL = 56, TW = 40, TH = 28, TOP = 4, LEFT = 4, S = 1.16;
  const LW = Math.round(TW * S), LH = Math.round(TH * S);
  let checked = false, p = 0, dragging = false, snapping = false, moved = false, startX = 0, startP = 0;
  const lens = mountGlassLens({
    target: iswTrack,
    host: isw,
    lensW: LW,
  ...
  const snapTo = (target) => {
    ...
    snapping = true;
    glass();
    const from = p, t0 = performance.now(), dur = 320;
    const step = (now) => {
      const k = Math.min(1, (now - t0) / dur);
...
if (svgBtn && svgBg) {
  const ripple = mountSvgRipple({ target: svgBg, host: svgBtn, maxFrac: 0.85, strength: 24, chroma: 0.4, spec: 0.7, blur: 0.4 });
  ...
  cfgSections.push({ id: "ripple", label: "Ripple button", icon: CFG_ICONS.ripple, params: RIPPLE_PARAMS, opts: ripple.getOptions(), apply: (patch) => ripple.reconfigure(patch) });
}
...
if (qrMount) {
  let qr = null;
  const mountQR = () => {
    qr = mountGlassQR(qrMount, { value: "https://principlestash.com", size: 300, errorCorrectionLevel: "Q" });
  ...
  cfgSections.push({
    ...
    icon: CFG_ICONS.qr,
    params: QR_PARAMS,
    opts: { scaleX: 0.08, scaleY: 0.08, chromaAmount: 1, eyeRefractionScale: 0.16, lensDepth: 30, lensDuration: 6e3, colorSplash: 300, ringStart: 0.15, ringEnd: 0.9 },
...
if (gmBtnEl) {
  const btn = mountGlassButton(gmBtnEl, { ...GLASS_SURFACE_DEFAULTS, strength: 18, dome: 13, chroma: 0.42, pulse: 0.55 });
  ...
  const ICON = {
    ...
    pause: svg('<rect x="4.3" y="3.4" width="2.6" height="9.2" rx="0.7" fill="currentColor" stroke="none"/><rect x="9.1" y="3.4" width="2.6" height="9.2" rx="0.7" fill="currentColor" stroke="none"/>'),
    ...
    cart: svg('<path d="M1.8 2.4h1.7l1.4 7.4h6.2l1.3-5.2H4.7"/><circle cx="6.1" cy="13" r="1.05" fill="currentColor" stroke="none"/><circle cx="11" cy="13" r="1.05" fill="currentColor" stroke="none"/>')
  };
  const SPINNER = svg('<circle cx="8" cy="8" r="5.6" stroke-opacity="0.32"/><path d="M13.6 8A5.6 5.6 0 0 0 8 2.4" stroke-linecap="round"/>', "gm-spin");
...
if (gmDdEl) {
  ...
  if (trigger && menu) {
    const dd = mountGlassDropdown({ trigger, menu, ...GLASS_SURFACE_DEFAULTS, strength: 22, dome: 15, chroma: 0.45, blur: 0.5 });
    menu.querySelectorAll(".gm-dd__item").forEach((it) => it.addEventListener("click", () => dd.close()));
    cfgSections.push({
      id: "dropdown",
      label: "Dropdown",
...
if (gshapeStage) {
  ...
  const drawMeme = (canvas) => {
    ...
    const phrases = [
      ["such glass", "#e9ff2f", 32, 40, -9, 16], ["very svg", "#37ffd0", 100, 72, 7, 16],
      ["wow", "#ff5db1", 44, 116, -4, 19], ["so refract", "#ffffff", 104, 32, 11, 13], ["much dome", "#ffe14d", 66, 92, 3, 14]
    ];
  ...
  const shapeOpts = { ...GLASS_SHAPE_DEFAULTS, strength: 11, bevel: 3.2, dome: 5, edge: 1, glow: 0.4 };
  const shapes = [];
  ...
  if (memeCanvas) {
    drawMeme(memeCanvas);
    shapes.push(mountGlassShape({ target: memeCanvas, host: gshapeStage, source: memeCanvas, ...shapeOpts, dome: 8 }));
  }
  ...
  if (orbCanvas) {
    ...
    const orbEmojis = [
      { e: "🌊", a0: 0.0, rad: 7, s: 50 }, { e: "🎉", a0: 0.4, rad: 38, s: 38 },
      { e: "💎", a0: 1.9, rad: 40, s: 36 }, { e: "🔥", a0: 3.1, rad: 36, s: 40 },
      { e: "✨", a0: 4.4, rad: 42, s: 34 }, { e: "💧", a0: 5.4, rad: 33, s: 30 },
      { e: "🫧", a0: 2.6, rad: 44, s: 30 }, { e: "🌈", a0: 1.1, rad: 28, s: 32 }
    ];
    const drawOrb = (t) => {
    ...
    drawOrb(0);
    mountGlassLens({ target: orbCanvas, host: gshapeStage, lensW: 150, lensH: 150, radius: 75, dome: 16, depth: 10, strength: 15, chroma: 0.3, edge: 0.9, glow: 0.35, blur: 0.4, shade: 0 });
    if (!reduce) {
      let orbVisible = true, orbRaf = 0;
      const loop = (now) => {
        orbRaf = 0;
      ...
      new IntersectionObserver((es) => { orbVisible = es[0].isIntersecting; if (orbVisible && !orbRaf) orbRaf = requestAnimationFrame(loop); }, { rootMargin: "120px" }).observe(orbCanvas);
...
if (perfEl) {
  ...
  const CW = 158, CH = 100;
  const cards = [];
  let source = "dom", animate = false, glassOn = true, visible = false, peakFps = 0;

  // identical lens config regardless of source — only the target type differs
  const LENS = { lensW: CW, lensH: CH, radius: 16, dome: 8, depth: 7, strength: 10, chroma: 0.32, edge: 0.85, glow: 0.32, blur: 1.5, shade: 0 };

  // canvas source: paint a faux-UI once (static) or per frame (animate)
  const paintCanvas = (c, t) => {
    const ctx = c.ctx, a = t * 0.001 + c.phase;
    ...
    ctx.beginPath(); ctx.arc(26, 28, 12, 0, 6.2832); ctx.fill();
    ctx.fillRect(48, 22, 82, 7); ctx.globalAlpha = 0.6; ctx.fillRect(48, 36, 56, 6);
    ctx.globalAlpha = 0.5; for (let i = 0; i < 3; i++) ctx.fillRect(18, 60 + i * 12, CW - 36 - (i === 2 ? 46 : 0), 6);
    ctx.globalAlpha = 1;
  };

  const makeCard = (i) => {
    const el = document.createElement("div"); el.className = "perfcard";
    const hue = (i * 47) % 360;
    let target, ctx = null;
    if (source === "canvas") {
      const canvas = document.createElement("canvas");
      canvas.width = Math.round(CW * pdpr); canvas.height = Math.round(CH * pdpr);
      canvas.style.width = CW + "px"; canvas.style.height = CH + "px";
      ctx = canvas.getContext("2d"); ctx.setTransform(pdpr, 0, 0, pdpr, 0, 0);
      el.appendChild(canvas); target = canvas;
    } else {
      const content = document.createElement("div"); content.className = "perfcard__content";
      ...
      el.appendChild(content); target = content;
    }
  ...
  const churn = (c, now) => {
    ...
    else c.target.style.backgroundPosition = ((Math.sin(now * 0.001 + c.phase) * 0.5 + 0.5) * 100).toFixed(1) + "% 50%";
  };

  const clearCards = () => { for (const c of cards) { if (c.lens) { try { c.lens.dispose(); } catch (e) {} } c.el.remove(); } cards.length = 0; };
  const setCount = (n) => {
    while (cards.length < n) { const c = makeCard(cards.length); scene.appendChild(c.el); cards.push(c); }
    while (cards.length > n) { const c = cards.pop(); if (c.lens) { try { c.lens.dispose(); } catch (e) {} } c.el.remove(); }
  };
  const rebuild = () => { const n = cards.length || (+(countRange && countRange.value) || 8); clearCards(); setCount(n); };
  ...
  const loop = (now) => {
    ...
    if (last && now - last > 400) { winStart = 0; last = 0; } // stall / tab-switch guard
    last = now;
    if (animate) for (const c of cards) churn(c, now);
    frames++;
    if (!winStart) { winStart = now; frames = 0; }
    else {
  ...
  new IntersectionObserver((es) => {
    ...
    if (visible && !rafId) { winStart = 0; last = 0; rafId = requestAnimationFrame(loop); } // restart on entry
  }, { rootMargin: "80px" }).observe(perfEl);
  ...
  if (animChk) animChk.addEventListener("change", () => { animate = animChk.checked; perfEl.classList.toggle("is-animating", animate); winStart = 0; });
...
function buildTuner(sections) {
  ...
  let active = saved.active != null && sections[saved.active] ? sections[saved.active] : sections[0];
  const save = () => {
    const data = { sections: Object.fromEntries(sections.map((s) => [s.id, s.opts])), pos, active: sections.indexOf(active) };
    try {
  ...
  const sectionJSON = () => "{ " + active.params.map((p) => `${p.k}: ${fmt(active.opts[p.k])}`).join(", ") + " }";
  ...
  panel.querySelector(".cfg__min").addEventListener("click", () => panel.classList.toggle("is-min"));
  ...
  bar.addEventListener("pointermove", (e) => {
    if (!drag)
      return;
    const nx = Math.max(6, Math.min(window.innerWidth - panel.offsetWidth - 6, ox + e.clientX - sx));
    const ny = Math.max(6, Math.min(window.innerHeight - panel.offsetHeight - 6, oy + e.clientY - sy));
...
(() => {
  ...
  const rd = (k, d) => { const v = parseFloat(base.dataset[k]); return Number.isNaN(v) ? d : v; };
  cfgSections.push({
    id: "paths",
    label: "Render paths",
    icon: CFG_ICONS.paths,
    params: RENDER_PARAMS,
    opts: { strength: rd("strength", 16), chroma: rd("chroma", 0.3), blur: rd("blur", 2), dome: rd("dome", 14), depth: rd("depth", 20), edge: rd("edge", 0.8), glow: rd("glow", 0.2) },
    focus: 0,
    focuses: [
      { id: "svg", label: "SVG", el: els.svg, dead: [] },
      { id: "webgl", label: "WebGL", el: els.webgl, dead: ["edge", "glow"] },
      { id: "frost", label: "Frost", el: els.frost, dead: ["strength", "chroma", "dome", "depth", "edge", "glow"] }
    ],
    apply(patch) {
...
const TUNER_ORDER = ["paths", "shape", "lens", "font", "segmented", "ripple", "button", "dropdown", "qr"];

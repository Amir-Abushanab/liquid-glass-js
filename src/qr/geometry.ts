...
export function buildQRGeometry({
  ...
  const flat = Array.prototype.slice.call(QRCode.create(value, { errorCorrectionLevel: ec }).modules.data, 0);
  ...
  [{ x: 0, y: 0 }, { x: 1, y: 0 }, { x: 0, y: 1 }].forEach(({ x, y }) => {
  ...
  matrix.forEach((row, t) => {
    row.forEach((bit, r) => {
      const inFinder =
        (t < 7 && r < 7) ||
        (t > matrix.length - 8 && r < 7) ||
        (t < 7 && r > matrix.length - 8);

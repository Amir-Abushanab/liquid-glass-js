...
export function renderDisplacementMap(o: GlassMapOptions): HTMLCanvasElement {
  ...
  const writePixel = (
    tr: number, tc: number, inside: boolean, dist: number, i: number, umag: number, mmag: number,
  ) => {
    const t = (tr * cw + tc) * 4;
    if (inside) {
      const x = tc + 0.5 - cw / 2;
      ...
      img.data[t] = Math.round((0.5 - 0.5 * u * i) * 255);
      img.data[t + 1] = Math.round((0.5 - 0.5 * m * i) * 255);
      if (specOn) {
        const lin = Math.abs(Math.max(-1, Math.min(1, x / halfW)) * ck + Math.max(-1, Math.min(1, y / halfH)) * sk);
        let r = 0;
        if (glow > 0) r += glow * Math.pow(GT > 0.001 ? Math.min(1, Math.max(0, lin - GI) / GT) : 0, glowExp) * i;
        if (edge > 0) r += edge * Math.max(0, 1 + dist / edgeW) * Math.pow(lin, edgeExp);
        if (r > 1) r = 1;
        img.data[t + 2] = Math.round(127 * r + 128);
      } else {
        img.data[t + 2] = 128;
      }
    } else {
  ...
  for (let row = 0; row < halfRows; row++) {
    ...
    for (let col = 0; col < halfCols; col++) {
      ...
      if (inside) {
        ...
        const ex = gx - iw + v;
        const ey = gy - ih + v;
        const d2 = Math.sqrt(Math.max(ex, 0) ** 2 + Math.max(ey, 0) ** 2) + Math.min(Math.max(ex, ey), 0) - v;
        i = 0.5 * (1 + erf(d2 * E));
      }

...
function supportsP3(): boolean {
  ...
  try {
    const c = document
      ...
      .getContext('2d', { colorSpace: 'display-p3' } as any);
...
export class PaintingTexture {
  ...
  updateClearColor(c: string) {
    this.clearColor = c;
  }
  updateRingStart(v: number) {
    this.ringStart = v;
  }
  updateRingEnd(v: number) {
    this.ringEnd = v;
  }
  updateSplashSpeed(v: number) {
    this.splashSpeed = v;
  } // read fresh in onClick()
  updateMousePosition(p: { x: number; y: number }) {

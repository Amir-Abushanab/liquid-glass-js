...
.ps-qr__stage {
  position: relative; border-radius: var(--ps-qr-radius, 56px); padding: 0.75rem;
...
/* Inset by the card's padding so the two curves stay concentric — and clamped at
   0, since --ps-qr-radius: 0 (a squared-off card) would otherwise go negative. */
.ps-qr__box {
  position: relative; overflow: hidden;
  border-radius: max(0px, calc(var(--ps-qr-radius, 56px) - 0.75rem));
}
